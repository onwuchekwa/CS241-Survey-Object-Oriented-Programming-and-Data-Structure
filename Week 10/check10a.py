...
def sort(numbers):
    """
    Fill in this method to sort the list of numbers using Selection Sort
    """
    for tempNum in range(len(numbers)-1, 0, -1):
      maxNumber = 0
      for location in range(1, tempNum + 1):
        if numbers[location] > numbers[maxNumber]:
          maxNumber = location
      temp = numbers[tempNum]
      numbers[tempNum] = numbers[maxNumber]
      numbers[maxNumber] = temp
